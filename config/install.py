...
class Installer:
  ...
  def fixConfFile(self, src):
    ...
    for line in oldFile.readlines():
      ...
      line = re.sub('\$\{SLEPC_DIR\}/\$\{PETSC_ARCH\}', self.installDir, line)
      line = re.sub('PETSC_ARCH=\$\{PETSC_ARCH\}', '', line)
      line = re.sub('\$\{SLEPC_DIR\}', self.installDir, line)
      lines.append(line)
    oldFile.close()
    newFile = open(src, 'w')
...
''')
  ...
  def outputInstallDone(self):
    print '''\
====================================
Install complete. It is useable with SLEPC_DIR=%s [and no more PETSC_ARCH].
Now to check if the libraries are working do (in current directory):
make SLEPC_DIR=%s PETSC_DIR=%s test
====================================\
''' % (self.installDir,self.installDir,self.petscDir)
    return
