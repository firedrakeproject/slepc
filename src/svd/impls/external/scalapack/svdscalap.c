/*
   - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   SLEPc - Scalable Library for Eigenvalue Problem Computations
   Copyright (c) 2002-2021, Universitat Politecnica de Valencia, Spain

   This file is part of SLEPc.
   SLEPc is distributed under a 2-clause BSD license (see LICENSE).
   - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
*/
/*
   This file implements a wrapper to the ScaLAPACK SVD solver
*/

#include <slepc/private/svdimpl.h>    /*I "slepcsvd.h" I*/
#include <slepc/private/slepcscalapack.h>

typedef struct {
  Mat As;        /* converted matrix */
} SVD_ScaLAPACK;

PetscErrorCode SVDSetUp_ScaLAPACK(SVD svd)
{
  SVD_ScaLAPACK  *ctx = (SVD_ScaLAPACK*)svd->data;
  PetscInt       M,N;

  PetscFunctionBegin;
  SVDCheckStandard(svd);
  CHKERRQ(MatGetSize(svd->A,&M,&N));
  svd->ncv = N;
  if (svd->mpd!=PETSC_DEFAULT) CHKERRQ(PetscInfo(svd,"Warning: parameter mpd ignored\n"));
  if (svd->max_it==PETSC_DEFAULT) svd->max_it = 1;
  svd->leftbasis = PETSC_TRUE;
  SVDCheckUnsupported(svd,SVD_FEATURE_STOPPING);
  CHKERRQ(SVDAllocateSolution(svd,0));

  /* convert matrix */
  CHKERRQ(MatDestroy(&ctx->As));
  CHKERRQ(MatConvert(svd->OP,MATSCALAPACK,MAT_INITIAL_MATRIX,&ctx->As));
  PetscFunctionReturn(0);
}

PetscErrorCode SVDSolve_ScaLAPACK(SVD svd)
{
  SVD_ScaLAPACK  *ctx = (SVD_ScaLAPACK*)svd->data;
  Mat            A = ctx->As,Z,Q,QT,U,V;
  Mat_ScaLAPACK  *a = (Mat_ScaLAPACK*)A->data,*q,*z;
  PetscScalar    *work,minlwork;
  PetscBLASInt   info,lwork=-1,one=1;
  PetscInt       M,N,m,n,mn;
#if defined(PETSC_USE_COMPLEX)
  PetscBLASInt   lrwork;
  PetscReal      *rwork,dummy;
#endif

  PetscFunctionBegin;
  CHKERRQ(MatGetSize(A,&M,&N));
  CHKERRQ(MatGetLocalSize(A,&m,&n));
  mn = (M>=N)? n: m;
  CHKERRQ(MatCreate(PetscObjectComm((PetscObject)A),&Z));
  CHKERRQ(MatSetSizes(Z,m,mn,PETSC_DECIDE,PETSC_DECIDE));
  CHKERRQ(MatSetType(Z,MATSCALAPACK));
  CHKERRQ(MatSetUp(Z));
  CHKERRQ(MatAssemblyBegin(Z,MAT_FINAL_ASSEMBLY));
  CHKERRQ(MatAssemblyEnd(Z,MAT_FINAL_ASSEMBLY));
  z = (Mat_ScaLAPACK*)Z->data;
  CHKERRQ(MatCreate(PetscObjectComm((PetscObject)A),&QT));
  CHKERRQ(MatSetSizes(QT,mn,n,PETSC_DECIDE,PETSC_DECIDE));
  CHKERRQ(MatSetType(QT,MATSCALAPACK));
  CHKERRQ(MatSetUp(QT));
  CHKERRQ(MatAssemblyBegin(QT,MAT_FINAL_ASSEMBLY));
  CHKERRQ(MatAssemblyEnd(QT,MAT_FINAL_ASSEMBLY));
  q = (Mat_ScaLAPACK*)QT->data;

  CHKERRQ(PetscFPTrapPush(PETSC_FP_TRAP_OFF));
#if !defined(PETSC_USE_COMPLEX)
  /* allocate workspace */
  PetscStackCallBLAS("SCALAPACKgesvd",SCALAPACKgesvd_("V","V",&a->M,&a->N,a->loc,&one,&one,a->desc,svd->sigma,z->loc,&one,&one,z->desc,q->loc,&one,&one,q->desc,&minlwork,&lwork,&info));
  PetscCheckScaLapackInfo("gesvd",info);
  CHKERRQ(PetscBLASIntCast((PetscInt)minlwork,&lwork));
  CHKERRQ(PetscMalloc1(lwork,&work));
  /* call computational routine */
  PetscStackCallBLAS("SCALAPACKgesvd",SCALAPACKgesvd_("V","V",&a->M,&a->N,a->loc,&one,&one,a->desc,svd->sigma,z->loc,&one,&one,z->desc,q->loc,&one,&one,q->desc,work,&lwork,&info));
  PetscCheckScaLapackInfo("gesvd",info);
  CHKERRQ(PetscFree(work));
#else
  /* allocate workspace */
  PetscStackCallBLAS("SCALAPACKgesvd",SCALAPACKgesvd_("V","V",&a->M,&a->N,a->loc,&one,&one,a->desc,svd->sigma,z->loc,&one,&one,z->desc,q->loc,&one,&one,q->desc,&minlwork,&lwork,&dummy,&info));
  PetscCheckScaLapackInfo("gesvd",info);
  CHKERRQ(PetscBLASIntCast((PetscInt)PetscRealPart(minlwork),&lwork));
  lrwork = 1+4*PetscMax(a->M,a->N);
  CHKERRQ(PetscMalloc2(lwork,&work,lrwork,&rwork));
  /* call computational routine */
  PetscStackCallBLAS("SCALAPACKgesvd",SCALAPACKgesvd_("V","V",&a->M,&a->N,a->loc,&one,&one,a->desc,svd->sigma,z->loc,&one,&one,z->desc,q->loc,&one,&one,q->desc,work,&lwork,rwork,&info));
  PetscCheckScaLapackInfo("gesvd",info);
  CHKERRQ(PetscFree2(work,rwork));
#endif
  CHKERRQ(PetscFPTrapPop());

  CHKERRQ(MatHermitianTranspose(QT,MAT_INITIAL_MATRIX,&Q));
  CHKERRQ(MatDestroy(&QT));
  CHKERRQ(BVGetMat(svd->U,&U));
  CHKERRQ(BVGetMat(svd->V,&V));
  if (M>=N) {
    CHKERRQ(MatConvert(Z,MATDENSE,MAT_REUSE_MATRIX,&U));
    CHKERRQ(MatConvert(Q,MATDENSE,MAT_REUSE_MATRIX,&V));
  } else {
    CHKERRQ(MatConvert(Q,MATDENSE,MAT_REUSE_MATRIX,&U));
    CHKERRQ(MatConvert(Z,MATDENSE,MAT_REUSE_MATRIX,&V));
  }
  CHKERRQ(BVRestoreMat(svd->U,&U));
  CHKERRQ(BVRestoreMat(svd->V,&V));
  CHKERRQ(MatDestroy(&Z));
  CHKERRQ(MatDestroy(&Q));

  svd->nconv  = svd->ncv;
  svd->its    = 1;
  svd->reason = SVD_CONVERGED_TOL;
  PetscFunctionReturn(0);
}

PetscErrorCode SVDDestroy_ScaLAPACK(SVD svd)
{
  PetscFunctionBegin;
  CHKERRQ(PetscFree(svd->data));
  PetscFunctionReturn(0);
}

PetscErrorCode SVDReset_ScaLAPACK(SVD svd)
{
  SVD_ScaLAPACK  *ctx = (SVD_ScaLAPACK*)svd->data;

  PetscFunctionBegin;
  CHKERRQ(MatDestroy(&ctx->As));
  PetscFunctionReturn(0);
}

SLEPC_EXTERN PetscErrorCode SVDCreate_ScaLAPACK(SVD svd)
{
  SVD_ScaLAPACK  *ctx;

  PetscFunctionBegin;
  CHKERRQ(PetscNewLog(svd,&ctx));
  svd->data = (void*)ctx;

  svd->ops->solve          = SVDSolve_ScaLAPACK;
  svd->ops->setup          = SVDSetUp_ScaLAPACK;
  svd->ops->destroy        = SVDDestroy_ScaLAPACK;
  svd->ops->reset          = SVDReset_ScaLAPACK;
  PetscFunctionReturn(0);
}
