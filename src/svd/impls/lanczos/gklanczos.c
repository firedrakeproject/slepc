...
PetscErrorCode computeres(SVD svd,PetscScalar sigma,Vec u,Vec v,PetscReal *norm)
{
  PetscErrorCode ierr;
  Vec            x,y;
...
PetscErrorCode SVDSolve_LANCZOS(SVD svd)
{
  ...
  for (svd->its=1;svd->its<svd->max_it;svd->its++) {
    ...
    for (i=svd->nconv;i<n;i++) {
      ...
      for (j=0;j<n;j++) {
	ierr = VecAXPY(svd->V[i],PT[j*n+i-svd->nconv],V[j]);CHKERRQ(ierr);
      }
      
      ierr = VecSet(svd->U[i],0.0);CHKERRQ(ierr);
      ierr = VecMAXPY(svd->U[i],n,Q+(i-svd->nconv)*n,U);CHKERRQ(ierr);
      
      ierr = computeres(svd,svd->sigma[i],svd->U[i],svd->V[i],&norm);CHKERRQ(ierr);
      printf("[%i] sigma[%i] = %g error = %g\n",svd->its,i,svd->sigma[i],norm);
      if (norm < svd->tol) {
        k++;
      } else break;
    }
