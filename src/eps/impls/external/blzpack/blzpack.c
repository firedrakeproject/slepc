...
PetscErrorCode EPSSetUp_BLZPACK(EPS eps)
{
  ...
  ierr = PetscLogObjectMemory(eps,(17+listor)*sizeof(PetscBLASInt));CHKERRQ(ierr);
...
lrstor*=10;
  ...
  ierr = PetscLogObjectMemory(eps,(4+lrstor)*sizeof(PetscReal));CHKERRQ(ierr);
  blz->rstor[3] = lrstor;
  ...
  ierr = PetscLogObjectMemory(eps,ncuv*eps->nloc*sizeof(PetscScalar));CHKERRQ(ierr);
  ...
  ierr = PetscLogObjectMemory(eps,2*eps->ncv*sizeof(PetscReal));CHKERRQ(ierr);
