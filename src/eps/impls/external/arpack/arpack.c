...
PetscErrorCode EPSSolve_ARPACK(EPS eps)
{
  ...
  do {
    ...
    } else if (ido != 99) SETERRQ(PetscObjectComm((PetscObject)eps),PETSC_ERR_LIB,"Internal error in ARPACK reverse communication interface (ido=%" PetscInt_FMT ")",ido);
...
#endif
    if (info!=0) SETERRQ(PetscObjectComm((PetscObject)eps),PETSC_ERR_LIB,"Error reported by ARPACK subroutine xxEUPD (%" PetscInt_FMT ")",info);
  }
