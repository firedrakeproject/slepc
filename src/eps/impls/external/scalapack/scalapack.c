...
PetscErrorCode EPSSolve_ScaLAPACK(EPS eps)
{
  ...
  PetscReal      rdummy,*gap=NULL,orfac=-1.0,*w = eps->errest;  /* used to store real eigenvalues */
  PetscScalar    *work,minlwork;
  PetscBLASInt   i,info,idummy,lwork=-1,liwork=-1,minliwork,*iwork,*ifail=NULL,*iclustr=NULL,one=1;
#if defined(PETSC_USE_COMPLEX)
  PetscReal      *rwork,minlrwork;
...
#if !defined(PETSC_USE_COMPLEX)
    /* allocate workspace */
    PetscStackCallBLAS("SCALAPACKsygvx",SCALAPACKsygvx_(&one,"V","A","L",&a->N,a->loc,&one,&one,a->desc,b->loc,&one,&one,b->desc,&rdummy,&rdummy,&idummy,&idummy,&rdummy,&idummy,&idummy,w,&orfac,q->loc,&one,&one,q->desc,&minlwork,&lwork,&minliwork,&liwork,ifail,iclustr,gap,&info));
    PetscCheckScaLapackInfo("sygvx",info);
    ierr = PetscBLASIntCast(minlwork,&lwork);CHKERRQ(ierr);
    liwork = minliwork;
    /* call computational routine */
    ierr = PetscMalloc5(lwork,&work,a->grid->nprow*a->grid->npcol,&gap,liwork,&iwork,a->N,&ifail,2*a->grid->nprow*a->grid->npcol,&iclustr);CHKERRQ(ierr);
    PetscStackCallBLAS("SCALAPACKsygvx",SCALAPACKsygvx_(&one,"V","A","L",&a->N,a->loc,&one,&one,a->desc,b->loc,&one,&one,b->desc,&rdummy,&rdummy,&idummy,&idummy,&rdummy,&idummy,&idummy,w,&orfac,q->loc,&one,&one,q->desc,work,&lwork,iwork,&liwork,ifail,iclustr,gap,&info));
    PetscCheckScaLapackInfo("sygvx",info);
    ierr = PetscFree5(work,gap,iwork,ifail,iclustr);CHKERRQ(ierr);
#else
    /* allocate workspace */
    PetscStackCallBLAS("SCALAPACKsygvx",SCALAPACKsygvx_(&one,"V","A","L",&a->N,a->loc,&one,&one,a->desc,b->loc,&one,&one,b->desc,&rdummy,&rdummy,&idummy,&idummy,&rdummy,&idummy,&idummy,w,&orfac,q->loc,&one,&one,q->desc,&minlwork,&lwork,&minlrwork,&lrwork,&minliwork,&liwork,ifail,iclustr,gap,&info));
    PetscCheckScaLapackInfo("sygvx",info);
    ierr = PetscBLASIntCast(minlwork,&lwork);CHKERRQ(ierr);
    ierr = PetscBLASIntCast(minlrwork,&lrwork);CHKERRQ(ierr);
    liwork = minliwork;
    /* call computational routine */
    ierr = PetscMalloc6(lwork,&work,lrwork,&rwork,a->grid->nprow*a->grid->npcol,&gap,liwork,&iwork,a->N,&ifail,2*a->grid->nprow*a->grid->npcol,&iclustr);CHKERRQ(ierr);
    PetscStackCallBLAS("SCALAPACKsygvx",SCALAPACKsygvx_(&one,"V","A","L",&a->N,a->loc,&one,&one,a->desc,b->loc,&one,&one,b->desc,&rdummy,&rdummy,&idummy,&idummy,&rdummy,&idummy,&idummy,w,&orfac,q->loc,&one,&one,q->desc,work,&lwork,rwork,&lrwork,iwork,&liwork,ifail,iclustr,gap,&info));
    PetscCheckScaLapackInfo("sygvx",info);
    ierr = PetscFree6(work,rwork,gap,iwork,ifail,iclustr);CHKERRQ(ierr);
#endif
