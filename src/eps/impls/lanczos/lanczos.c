...
#undef __FUNCT__  
#define __FUNCT__ "EPSSelectiveLanczos2"
/*
   EPSSelectiveLanczos2 - Selective reorthogonalization.
*/
static PetscErrorCode EPSSelectiveLanczos2(EPS eps,PetscScalar *T,Vec *V,int k,int *M,Vec f,PetscReal *beta,PetscTruth *breakdown,PetscReal anorm)
{
...
#endif
          VV[eps->nds+k+nritz] = eps->AV[nritz];
          nritz++;
	}
      }
    }

    ierr = EPSOrthogonalize(eps,eps->nds+k+nritz,VV,f,PETSC_NULL,&norm,breakdown);CHKERRQ(ierr);
    if (*breakdown) {
      *M = j+1;
      break;
    }
  ...
  *beta = norm;
  ...
  ierr = PetscFree(work);CHKERRQ(ierr);
  ierr = PetscFree(iwork);CHKERRQ(ierr);
  if (eps->nds+eps->ncv>100) { ierr = PetscFree(VV);CHKERRQ(ierr); }
  PetscFunctionReturn(0);
#endif
}
