...
PetscErrorCode EPSSolve_KRYLOVSCHUR(EPS eps)
{
  ...
  while (eps->reason == EPS_CONVERGED_ITERATING) {
    ...
    eps->nv = eps->ncv;
    ierr = EPSBasicArnoldi(eps,PETSC_FALSE,S,eps->V,eps->nconv+p,&eps->nv,u,&beta,&breakdown);CHKERRQ(ierr);
    ...
    p = PetscMax(PetscMin(eps->ncv/2,eps->nv-k-1),0);
    for (i=eps->nconv;i<k+p;i++) {
    ...
    if (eps->reason == EPS_CONVERGED_ITERATING && !breakdown) {
      ...
      for (i=k;i<k+p;i++) {
        S[i*eps->ncv+k+p] = Q[i*eps->nv+eps->nv-1]*beta;
      }
      ierr = VecCopy(u,eps->V[k+p]);CHKERRQ(ierr);
    }
