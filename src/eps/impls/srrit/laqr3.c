/* laqr3.f -- translated by f2c (version 20030320).
	...
	integer *ldz, doublereal *work, integer *info)
{
    ...
    static doublereal t1, t2, t3, v1, v2, v3, h00, h10, h11, h12;
    static integer ic;
    static doublereal h21, h22, h33, h44;
    static integer nh, in;
    static doublereal cs;
    static integer nr;
    static doublereal sn;
    static integer nz;
    static doublereal wr1, h33s, h44s;
    static integer itn, its;
    static doublereal ulp, sum, tst1, h43h34;
    static integer ierr;
    ...
    extern /* Subroutine */ int dlarfg_(integer *, doublereal *, doublereal *,
	     integer *, doublereal *), dlaexc_(logical *, integer *, 
...
/*        Look for a single small subdiagonal element. */
	...
	for (k = i__; k >= i__2; --k) {
	    ...
	    if (tst1 == 0.) {
		i__3 = i__ - l + 1;
		tst1 = dlanhs_("1", &i__3, &h__[l + l * h_dim1], ldh, &work[1]
			, (ftnlen)1);
	    }
/* Computing MAX */
	    d__2 = ulp * tst1;
...
/*        loop to position 1 x 1 block */
	...
	wr1 = (d__1 = h__[i__ + i__ * h_dim1], abs(d__1));
	ic = i__;
	in = i__ + 1;

L250:
	if (in == *ihi || h__[in + 1 + in * h_dim1] == 0.) {
	    if ((d__1 = h__[in + in * h_dim1], abs(d__1)) > wr1) {
		dlaexc_(wantz, n, &h__[h_offset], ldh, &z__[z_offset], ldz, &
			ic, &c__1, &c__1, &work[1], &ierr);
		if (ierr != 0) {
		    *info = *n + i__;
		    return 0;
		}
	    ...
	    temp = sqrt((d__1 = h__[in + 1 + in * h_dim1], abs(d__1))) * sqrt(
		    (d__2 = h__[in + (in + 1) * h_dim1], abs(d__2)));
	    if (dlapy2_(&h__[in + in * h_dim1], &temp) > wr1) {
		dlaexc_(wantz, n, &h__[h_offset], ldh, &z__[z_offset], ldz, &
			ic, &c__1, &c__2, &work[1], &ierr);
		if (ierr != 0) {
		    *info = *n + i__;
		    return 0;
		}
...
L230:
	    if (in == *ihi || h__[in + 1 + in * h_dim1] == 0.) {
		if ((d__1 = h__[in + in * h_dim1], abs(d__1)) > wr1) {
		    dlaexc_(wantz, n, &h__[h_offset], ldh, &z__[z_offset], 
			    ldz, &ic, &c__1, &c__1, &work[1], &ierr);
		    if (ierr != 0) {
			*info = *n + i__;
			return 0;
		    }
		...
		temp = sqrt((d__1 = h__[in + 1 + in * h_dim1], abs(d__1))) * 
			sqrt((d__2 = h__[in + (in + 1) * h_dim1], abs(d__2)));
		if (dlapy2_(&h__[in + in * h_dim1], &temp) > wr1) {
		    dlaexc_(wantz, n, &h__[h_offset], ldh, &z__[z_offset], 
			    ldz, &ic, &c__1, &c__2, &work[1], &ierr);
		    if (ierr != 0) {
			*info = *n + i__;
			return 0;
		    }
...
L240:
	    wr1 = (d__1 = h__[i__ - 1 + (i__ - 1) * h_dim1], abs(d__1));
	    ic = i__ - 1;
	    in = ic + 1;
...
/*           Complex conjugate eigenvalues, loop to position 2 x 2 block */
	    ...
	    d__3 = sqrt((d__1 = h__[i__ + (i__ - 1) * h_dim1], abs(d__1))) * 
		    sqrt((d__2 = h__[i__ - 1 + i__ * h_dim1], abs(d__2)));
	    wr1 = dlapy2_(&h__[i__ - 1 + (i__ - 1) * h_dim1], &d__3);
	    ic = i__ - 1;
	    in = ic + 2;
...
/*     eigenvalues to WR and WI */
	...
	if (h__[i__ + 1 + i__ * h_dim1] == 0.) {
	    ...
	    wr[i__] = h__[i__ + i__ * h_dim1];
	    wr[i__ + 1] = h__[i__ + i__ * h_dim1];
	    wi[i__] = sqrt((d__1 = h__[i__ + 1 + i__ * h_dim1], abs(d__1))) * 
