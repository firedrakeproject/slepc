...
#undef __FUNCT__
#define __FUNCT__ "PEPSimpleNRefSetUp2"
static PetscErrorCode PEPSimpleNRefSetUp2(PEP pep,PEPSimpNRefctx2 **ctx_)
{
  PetscErrorCode ierr;
...
*/
#undef __FUNCT__
#define __FUNCT__ "PEPSimpleNRefGatherEigenpair2"
static PetscErrorCode PEPSimpleNRefGatherEigenpair2(PEP pep,PEPSimpNRefctx2 *ctx,PetscInt sc,PetscInt idx,PetscInt *fail)
{
  PetscErrorCode    ierr;
  PetscMPIInt       nproc,p;
...
#undef __FUNCT__
#define __FUNCT__ "PEPSimpleNRefScatterEigenvector2"
static PetscErrorCode PEPSimpleNRefScatterEigenvector2(PEP pep,PEPSimpNRefctx2 *ctx,PetscInt sc,PetscInt idx)
{
  PetscErrorCode    ierr;
  Vec               v;
  const PetscScalar *array;
...
#undef __FUNCT__
#define __FUNCT__ "PEPEvaluateFunctionDerivatives2"
static PetscErrorCode PEPEvaluateFunctionDerivatives2(PEP pep,PetscScalar alpha,PetscScalar *vals)
{
...
#undef __FUNCT__
#define __FUNCT__ "PEPSimpleNRefSetUpSystem2"
static PetscErrorCode PEPSimpleNRefSetUpSystem2(PEP pep,PEPSimpNRefctx2 *ctx,Mat *A,PetscInt idx,Mat *T,PetscBool ini,Vec t,Vec v)
{
  PetscErrorCode    ierr;
  PetscInt          i,nmat=pep->nmat;
  PetscScalar       *coeffs;
  ...
  ierr = PEPEvaluateFunctionDerivatives2(pep,pep->eigr[idx],coeffs);CHKERRQ(ierr);
  ...
  for (i++;i<nmat;i++) {
    ...
    ierr = VecAXPY(w,coeffs[i],t);CHKERRQ(ierr);
  }
...
#undef __FUNCT__
#define __FUNCT__ "PEPNewtonRefinementSimple2"
PetscErrorCode PEPNewtonRefinementSimple2(PEP pep,PetscInt *maxits,PetscReal *tol,PetscInt k)
{
  PetscErrorCode     ierr;
  PetscInt           i,its,idx=0,*idx_sc,*its_sc,color,*fail_sc;
  KSP                ksp;
  Mat                T=NULL;
  Vec                v,dv,t[2],r;
  PetscScalar        deig,tt[2],ttt;
  ...
  ierr = PEPSimpleNRefSetUp2(pep,&ctx);CHKERRQ(ierr);
  ...
  while (!solved) {
    for (i=0;i<pep->npart;i++) {
      ...
      if (its_sc[i]==0) {
        ...
        if (idx_sc[i]>=k) {
          ...
          ierr = PEPSimpleNRefScatterEigenvector2(pep,ctx,i,idx_sc[i]);CHKERRQ(ierr);
        }
      }  else { /* Gather Eigenpair from subcommunicator i */
        ierr = PEPSimpleNRefGatherEigenpair2(pep,ctx,i,idx_sc[i],&fail_sc[i]);CHKERRQ(ierr);
      }
      while (sc_pend) {
        if (tol && !fail_sc[i]) {
        ...
        if (error<=*tol || its_sc[i]>=its || fail_sc[i]) {
          ...
          fail_sc[i] = 0;
          its_sc[i] = 0;
          if (idx_sc[i]<k) { ierr = PEPSimpleNRefScatterEigenvector2(pep,ctx,i,idx_sc[i]);CHKERRQ(ierr); }
        } else {
          sc_pend = PETSC_FALSE;
          its_sc[i]++;
        }
    ...
    if (idx_sc[color]<k) {
      ...
      ierr = PEPSimpleNRefSetUpSystem2(pep,ctx,ctx->A,idx_sc[color],&T,ini,t[0],v);CHKERRQ(ierr);
...
static PetscErrorCode PEPSimpleNRefSetUpSystem(PEP pep,Mat *A,PEPSimpNRefctx *ctx,PetscInt idx,Mat *M,Mat *T,PetscBool ini,Vec *t,Vec v)
{
  PetscErrorCode    ierr;
  ...
  if (size>1) {
    ...
    if (rank==size-1) {
      ierr = MatSetValues(*M,1,&mg,pep->n,cols2,array,INSERT_VALUES);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "PEPNewtonRefinementSimple_"
PetscErrorCode PEPNewtonRefinementSimple_(PEP pep,PetscInt *maxits,PetscReal *tol,PetscInt k)
{
  PetscErrorCode    ierr;
  ...
  while (!solved) {
    ...
    if (idx_sc[color]<k) {
      ...
      ierr = PEPSimpleNRefSetUpSystem(pep,ctx->A,ctx,idx_sc[color],&M,&T,ini,t,v);CHKERRQ(ierr);
...
PetscErrorCode PEPNewtonRefinementSimple(PEP pep,PetscInt *maxits,PetscReal *tol,PetscInt k)
{
  ...
  if (bem) {
    ierr = PEPNewtonRefinementSimple2(pep,maxits,tol,k);CHKERRQ(ierr);
  } else {
    ierr = PEPNewtonRefinementSimple_(pep,maxits,tol,k);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
