...
PetscErrorCode PEPNewtonRefinementSimple(PEP pep,PetscInt *maxits,PetscReal tol,PetscInt k)
{
  ...
  while (!solved) {
    ...
    if (idx_sc[color]<k) {
      ...
      PetscCheckFalse(pep->eigi[idx_sc[color]]!=0.0,PetscObjectComm((PetscObject)pep),PETSC_ERR_SUP,"Simple Refinement not implemented in real scalars for complex eigenvalues");
#endif
      if (pep->npart==1) {
      ...
      ierr = PEP_KSPSetOperators(pep->refineksp,T,P);CHKERRQ(ierr);
      if (ini) {
