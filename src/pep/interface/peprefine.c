...
static PetscErrorCode PEPSimpleNRefSetUpSystem(PEP pep,PEPSimpNRefctx *ctx,Mat *A,PetscInt idx,Mat *T,PetscBool ini,Vec t,Vec v)
{
  PetscErrorCode    ierr;
  PetscInt          i,nmat=pep->nmat;
  PetscScalar       *coeffs;
...
PetscErrorCode PEPNewtonRefinementSimple(PEP pep,PetscInt *maxits,PetscReal *tol,PetscInt k)
{
  ...
  while (!solved) {
    ...
    if (idx_sc[color]<k) {
      ...
      ierr = PEPSimpleNRefSetUpSystem(pep,ctx,ctx->A,idx_sc[color],&T,ini,t[0],v);CHKERRQ(ierr);
