...
int main(int argc,char **argv)
{
  ...
  CHKERRQ(PEPSetType(pep,PEPCISS));
  CHKERRQ(PEPGetRG(pep,&rg));
  CHKERRQ(RGSetType(rg,RGELLIPSE));
  CHKERRQ(RGEllipseSetParameters(rg,PetscCMPLX(-0.1,0.3),0.1,0.25));
  CHKERRQ(PEPCISSSetSizes(pep,24,PETSC_DEFAULT,PETSC_DEFAULT,1,PETSC_DEFAULT,PETSC_TRUE));
  CHKERRQ(PEPCISSGetKSPs(pep,&nsolve,&ksp));
  for (i=0;i<nsolve;i++) {
    CHKERRQ(KSPSetTolerances(ksp[i],1e-12,PETSC_DEFAULT,PETSC_DEFAULT,PETSC_DEFAULT));
    CHKERRQ(KSPSetType(ksp[i],KSPPREONLY));
  ...
  /* solve */
  CHKERRQ(PetscObjectTypeCompare((PetscObject)pep,PEPCISS,&flg));
  if (flg) {
    CHKERRQ(PEPCISSGetExtraction(pep,&ext));
