...
#undef __FUNCT__
#define __FUNCT__ "PEPEvaluateBasisforMatrix2"
/*
  Evaluates the first d elements of the polynomial basis
  on a given matrix H which is considered to be triangular
*/
static PetscErrorCode PEPEvaluateBasisforMatrix2(PEP pep,PetscInt nm,PetscInt k,PetscScalar *H,PetscInt ldh,PetscScalar *fH)
{
  PetscErrorCode ierr;
...
#undef __FUNCT__
#define __FUNCT__ "NRefRightSide2"
/*
   Computes the residual P(H,V*S)*e_j for the polynomial
*/
static PetscErrorCode NRefRightSide2(PetscInt nmat,PetscReal *pcf,Mat *A,PetscInt k,BV V,PetscScalar *S,PetscInt lds,PetscInt j,PetscScalar *H,PetscInt ldh,PetscScalar *fH,PetscScalar *DfH,PetscScalar *dH,BV dV,PetscScalar *dVS,PetscInt rds,Vec Rv,PetscScalar *Rh,BV W,Vec t)
{
  PetscErrorCode ierr;
...
#undef __FUNCT__
#define __FUNCT__ "NRefSysSolveBEM2"
static PetscErrorCode NRefSysSolveBEM2(PetscInt k,PetscInt sz,BV W,PetscScalar *w,BV Wt,PetscScalar *wt,PetscScalar *d,PetscScalar *dt,KSP ksp,BV T2,BV T3 ,PetscScalar *T4,PetscBool trans,Vec x1,PetscScalar *x2,Vec sol1,PetscScalar *sol2,Vec vw)
{
  PetscErrorCode ierr;
...
#undef __FUNCT__
#define __FUNCT__ "NRefSysSetup_explicit2"
static PetscErrorCode NRefSysSetup_explicit2(PEP pep,PetscInt k,KSP ksp,PetscScalar *fH,PetscScalar *S,PetscInt lds,PetscScalar *fh,PetscScalar h,BV V,MatExplicitCtx2 *matctx)
{
  PetscErrorCode    ierr;
  ...
  for (i=0;i<k;i++) {
    ...
    ierr = NRefSysSolveBEM2(k,i,W,w,Wt,wt,d,dt,ksp,M2,M3,M4,PETSC_FALSE,vc,M4+i*k,vc2,w+i*k,matctx->t);CHKERRQ(ierr);
    ...
    ierr = NRefSysSolveBEM2(k,i,W,w,Wt,wt,d,dt,ksp,M2,M3,M4,PETSC_TRUE,vc,Ts,vc2,wt+i*k,matctx->t);CHKERRQ(ierr);
    ierr = BVRestoreColumn(M3,i,&vc);CHKERRQ(ierr);
    ierr = BVGetColumn(M2,i,&vc);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "NRefSysSolve_explicit2"
static PetscErrorCode NRefSysSolve_explicit2(PetscInt k,KSP ksp,Vec Rv,PetscScalar *Rh,Vec dVi,PetscScalar *dHi,MatExplicitCtx2 *matctx)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;
   ierr = NRefSysSolveBEM2(k,k,matctx->W,matctx->w,matctx->Wt,matctx->wt,matctx->d,matctx->dt,ksp,matctx->M2,matctx->M3 ,matctx->M4,PETSC_FALSE,Rv,Rh,dVi,dHi,matctx->t);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

#undef __FUNCT__
#define __FUNCT__ "NRefSysIter_explicit2"
static PetscErrorCode NRefSysIter_explicit2(PetscInt i,PEP pep,PetscInt k,KSP ksp,PetscScalar *fH,PetscScalar *S,PetscInt lds,PetscScalar *fh,PetscScalar *H,PetscInt ldh,Vec Rv,PetscScalar *Rh,BV V,Vec dVi,PetscScalar *dHi,MatExplicitCtx2 *matctx)
{
  PetscErrorCode    ierr;
  ...
  if (!matctx->subc) {
    ...
    R   = Rv;
    Vi  = dVi;
    ierr = NRefSysSetup_explicit2(pep,k,ksp,fH,S,lds,fh,h,V,matctx);CHKERRQ(ierr);
  } else {
    if (i%matctx->subc->n==0 && (idx=i+matctx->subc->color)<k) {
      for (j=0;j<pep->nmat;j++) fh[j] = fH[j*k+idx+idx*lda];
      h = H[idx+idx*ldh];
      matctx->idx = idx;
      ierr = NRefSysSetup_explicit2(pep,k,ksp,fH,S,lds,fh,h,matctx->V,matctx);CHKERRQ(ierr);
    } else idx = matctx->idx;
  ...
  if (idx==i && idx<k) {
    ierr = NRefSysSolve_explicit2(k,ksp,R,Rh,Vi,dHi,matctx);CHKERRQ(ierr);
  }
  if (matctx->subc) {
...
#undef __FUNCT__
#define __FUNCT__ "PEPNRefForwardSubstitution2"
static PetscErrorCode PEPNRefForwardSubstitution2(PEP pep,PetscInt k,PetscScalar *S,PetscInt lds,PetscScalar *H,PetscInt ldh,PetscScalar *fH,BV dV,PetscScalar *dVS,PetscInt *rds,PetscScalar *dH,PetscInt lddh,KSP ksp,MatExplicitCtx2 *matctx)
{
  PetscErrorCode ierr;
  ...
  *rds = 0;
  ...
  for (i=0;i<k;i++) {
    ...
    ierr = NRefRightSide2(nmat,pep->pbc,At,k,pep->V,S,lds,i,H,ldh,fH,DfH,dH,dV,dVS,*rds,Rv,Rh,W,t);CHKERRQ(ierr);

    /* Update and solve system */
    ierr = BVGetColumn(dV,i,&dvi);CHKERRQ(ierr);
    ierr = NRefSysIter_explicit2(i,pep,k,ksp,fH,S,lds,fh,H,ldh,Rv,Rh,pep->V,dvi,dH+i*k,matctx);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "NRefOrthogStep2"
static PetscErrorCode NRefOrthogStep2(PEP pep,PetscInt k,PetscScalar *H,PetscInt ldh,PetscScalar *fH,PetscScalar *S,PetscInt lds,PetscInt *prs)
{
  PetscErrorCode ierr;
  ...
  ldg = deg*k;
  ierr = PEPEvaluateBasisforMatrix2(pep,nmat,k,H,ldh,fH);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "PEPNRefUpdateInvPair2"
static PetscErrorCode PEPNRefUpdateInvPair2(PEP pep,PetscInt k,PetscScalar *H,PetscInt ldh,PetscScalar *fH,PetscScalar *dH,PetscScalar *S,PetscInt lds,BV dV,PetscScalar *dVS,PetscInt rds)
{
  PetscErrorCode ierr;
  ...
  ierr = MatDestroy(&M0);CHKERRQ(ierr);
  ierr = NRefOrthogStep2(pep,k,H,ldh,fH,S,lds,&k);CHKERRQ(ierr);
  ierr = PetscFree2(tau,work);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

#undef __FUNCT__
#define __FUNCT__ "PEPNRefSetUp2"
static PetscErrorCode PEPNRefSetUp2(PEP pep,PetscInt k,PetscScalar *H,PetscInt ldh,MatExplicitCtx2 *matctx,PetscBool ini)
{
  PetscErrorCode    ierr;
...
#undef __FUNCT__
#define __FUNCT__ "NRefSubcommSetup2"
static PetscErrorCode NRefSubcommSetup2(PEP pep,PetscInt k,MatExplicitCtx2 *matctx,PetscInt nsubc)
{
  PetscErrorCode    ierr;
...
#undef __FUNCT__
#define __FUNCT__ "NRefSubcommDestroy2"
static PetscErrorCode NRefSubcommDestroy2(PEP pep,MatExplicitCtx2 *matctx)
{
  PetscErrorCode ierr;
  PetscInt       i;
...
#undef __FUNCT__
#define __FUNCT__ "PEPNewtonRefinement_TOAR2"
PetscErrorCode PEPNewtonRefinement_TOAR2(PEP pep,PetscScalar sigma,PetscInt *maxits,PetscReal *tol,PetscInt k,PetscScalar *S,PetscInt lds,PetscInt *prs)
{
  PetscErrorCode ierr;
  ...
  ierr = NRefOrthogStep2(pep,k,H,ldh,fH,S,lds,prs);CHKERRQ(ierr);
  ...
  if (nsubc>k) SETERRQ(PetscObjectComm((PetscObject)pep),1,"Amount of subcommunicators should not be larger than the invariant pair's dimension");
  ierr = BVSetActiveColumns(pep->V,0,k);CHKERRQ(ierr);
  ...
  if (nsubc>1) { /* spliting in subcommunicators */
    ...
    ierr = NRefSubcommSetup2(pep,k,matctx,nsubc);CHKERRQ(ierr);
  } else {
    matctx->subc=NULL;
  }
  ...
  for (i=0;i<its;i++) {
    ...
    ierr = PEPEvaluateBasisforMatrix2(pep,nmat,k,H,ldh,fH);
    ierr = PEPNRefSetUp2(pep,k,H,ldh,matctx,i==0?PETSC_TRUE:PETSC_FALSE);CHKERRQ(ierr);
    /* Solve the linear system */
    ierr = PEPNRefForwardSubstitution2(pep,k,S,lds,H,ldh,fH,dV,dVS,&rds,dH,k,pep->refineksp,matctx);CHKERRQ(ierr);
    /* Update X (=V*S) and H, and orthogonalize [X;X*fH1;...;XfH(deg-1)] */
    ierr = PEPNRefUpdateInvPair2(pep,k,H,ldh,fH,dH,S,lds,dV,dVS,rds);CHKERRQ(ierr);    
  }
  ierr = DSRestoreArray(pep->ds,DS_MAT_A,&H);CHKERRQ(ierr);  
  if (!flg && sinvert) {
  ...
  if (nsubc>1) {
    ierr = NRefSubcommDestroy2(pep,matctx);CHKERRQ(ierr);
  } 
...
static PetscErrorCode NRefSysIter_shell(PEP pep,PetscInt k,KSP ksp,PetscScalar *fH,PetscScalar *S,PetscInt lds,PetscScalar *fh,PetscScalar h,Vec Rv,PetscScalar *Rh,Vec dVi,PetscScalar *dHi)
{
  PetscErrorCode ierr;
  PetscInt       nmat=pep->nmat;
  FSubctx        *ctx;
  Mat            M;
  ...
  /* Solve system */
  ierr = NRefSysSolve_shell(ksp,nmat,Rv,Rh,k,dVi,dHi);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
  ...
  *rds = 0;
  ...
  for (i=0;i<k;i++) {
    ...
    if (matctx) {
      ...
      ierr = NRefSysIter_shell(pep,k,ksp,fH,S,lds,fh,h,Rv,Rh,dvi,dH+i*k);CHKERRQ(ierr);
...
#undef __FUNCT__
#define __FUNCT__ "PEPNewtonRefinement_TOAR_"
PetscErrorCode PEPNewtonRefinement_TOAR_(PEP pep,PetscScalar sigma,PetscInt *maxits,PetscReal *tol,PetscInt k,PetscScalar *S,PetscInt lds,PetscInt *prs)
{
  PetscErrorCode ierr;
  PetscScalar    *H,*work,*dH,*fH,*dVS;
...
{
  PetscErrorCode ierr;
  PetscBool bem=PETSC_FALSE;
  
  PetscFunctionBegin;
  ierr = PetscOptionsGetBool(NULL,"-pep_refine_bem",&bem,NULL);CHKERRQ(ierr);
  if (bem) {
    ierr = PEPNewtonRefinement_TOAR2(pep,sigma,maxits,tol,k,S,lds,prs);CHKERRQ(ierr);
  } else {
    ierr = PEPNewtonRefinement_TOAR_(pep,sigma,maxits,tol,k,S,lds,prs);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
