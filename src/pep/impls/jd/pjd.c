    ...
    for (pp=0;pp<pep->nmat;pp++) {
      ...
      if (nconv) {
        ...
        for (i=pp+1;i<pep->nmat;i++) {
          ...
          if (i!=pep->nmat-1) {
            PetscStackCallBLAS("BLAStrmv",BLAStrmv_("U","N","N",&n,pjd->T,&ld,tt,&one));
          }
        }
        if (rk==np-1 && pp<deg) {
          for (j=0;j<nconv;j++) xx[j] = x2[j];
          y2 = array2+nloc;
          ierr = BVDotVec(pjd->X,t1,y2);CHKERRQ(ierr);
...
static PetscErrorCode PEPJDUpdateExtendedPC(PEP pep,PetscScalar theta)
{
  ...
  if (n) { 
    ...
    ierr = PetscMalloc6(n,&q,n,&h,n,&p,n,&work,n*n,&A,n*n,&B);CHKERRQ(ierr);
    ...
    ierr = PetscMemzero(A,n*n*sizeof(PetscScalar));CHKERRQ(ierr);
