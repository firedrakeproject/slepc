...
static PetscErrorCode SVD_S_CAA(PEP pep,BV S,PetscScalar *pA,PetscInt *K)
{
  PetscErrorCode ierr;
  PEP_CISS       *ctx = (PEP_CISS*)pep->data;
  ...
  /* Compute QR factorizaton of S */
  ierr = BVGetSizes(S,NULL,&n,NULL);CHKERRQ(ierr);
  if (n<ml) SETERRQ(PetscObjectComm((PetscObject)pep),PETSC_ERR_SUP,"This extraction strategy does not support problem size n < m*L");
...
PetscErrorCode PEPSetUp_CISS(PEP pep)
{
  ...
  if (pep->max_it==PETSC_DEFAULT) pep->max_it = 1;
    ...
    for (inner=0;inner<=ctx->refine_inner;inner++) {
      if (ctx->extraction == PEP_CISS_EXTRACTION_HANKEL) {
        ...
        if (ctx->extraction == PEP_CISS_EXTRACTION_CAA) {
          ...
          ierr = SVD_S_CAA(pep,ctx->S,H0,&nv);CHKERRQ(ierr);
        } else {
          ierr = SVD_S(pep,ctx->S,H0,&nv);CHKERRQ(ierr);
        }
      }
