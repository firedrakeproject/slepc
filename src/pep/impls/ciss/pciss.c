...
static PetscErrorCode PEPComputeFunction(PEP pep,PetscScalar lambda,Mat T,Mat P,PetscBool deriv)
{
  ...
  if (!deriv && T != P) {
    ierr = STGetSplitPreconditionerInfo(pep->st,NULL,&strp);CHKERRQ(ierr);
    ierr = MatZeroEntries(P);CHKERRQ(ierr);
  }
  i = deriv?1:0;
  for (;i<pep->nmat;i++) {
    ierr = MatAXPY(T,coeff[i],A[i],str);CHKERRQ(ierr);
    if (!deriv && T != P) { ierr = MatAXPY(P,coeff[i],K[i],strp);CHKERRQ(ierr); }
  }
...
PetscErrorCode PEPSetUp_CISS(PEP pep)
{
  ...
  PetscCheckFalse(pep->which!=PEP_ALL,PetscObjectComm((PetscObject)pep),PETSC_ERR_SUP,"This solver supports only computing all eigenvalues");
  ...
  PetscCheckFalse(istrivial,PetscObjectComm((PetscObject)pep),PETSC_ERR_SUP,"CISS requires a nontrivial region, e.g. -rg_type ellipse ...");
  ierr = RGGetComplement(pep->rg,&flg);CHKERRQ(ierr);
  PetscCheckFalse(flg,PetscObjectComm((PetscObject)pep),PETSC_ERR_SUP,"A region with complement flag set is not allowed");
  ierr = PetscObjectTypeCompare((PetscObject)pep->rg,RGELLIPSE,&isellipse);CHKERRQ(ierr);
  PetscCheckFalse(!isellipse,PetscObjectComm((PetscObject)pep),PETSC_ERR_SUP,"Currently only implemented for elliptic regions");
  ...
  if (nsplit) {
    ...
    ierr = PetscMalloc1(nsplit,&ctx->Psplit);CHKERRQ(ierr);
    for (i=0;i<nsplit;i++) {
      ierr = STGetSplitPreconditionerTerm(pep->st,i,&ctx->Psplit[i]);CHKERRQ(ierr);
    }
  }
...
static PetscErrorCode PEPCISSSetSizes_CISS(PEP pep,PetscInt ip,PetscInt bs,PetscInt ms,PetscInt npart,PetscInt bsmax,PetscBool realmats)
{
  ...
  if (ip == PETSC_DECIDE || ip == PETSC_DEFAULT) {
    ...
    PetscCheckFalse(ip<1,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The ip argument must be > 0");
    PetscCheckFalse(ip%2,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The ip argument must be an even number");
  ...
  if (bs == PETSC_DECIDE || bs == PETSC_DEFAULT) {
    ...
    PetscCheckFalse(bs<1,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The bs argument must be > 0");
    ctx->L = bs;
  }
  oM = ctx->M;
  if (ms == PETSC_DECIDE || ms == PETSC_DEFAULT) {
    ...
    PetscCheckFalse(ms<1,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The ms argument must be > 0");
    PetscCheckFalse(ms>ctx->N,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The ms argument must be less than or equal to the number of integration points");
    ctx->M = PetscMax(ms,2);
  }
  onpart = ctx->npart;
  ...
  if (bsmax == PETSC_DECIDE || bsmax == PETSC_DEFAULT) {
    ...
    PetscCheckFalse(bsmax<1,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The bsmax argument must be > 0");
...
static PetscErrorCode PEPCISSSetThreshold_CISS(PEP pep,PetscReal delta,PetscReal spur)
{
  ...
  if (delta == PETSC_DEFAULT) {
    ...
    PetscCheckFalse(delta<=0.0,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The delta argument must be > 0.0");
    ctx->delta = delta;
  }
  if (spur == PETSC_DEFAULT) {
    ctx->spurious_threshold = PetscSqrtReal(SLEPC_DEFAULT_TOL);
  } else {
    PetscCheckFalse(spur<=0.0,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The spurious threshold argument must be > 0.0");
...
static PetscErrorCode PEPCISSSetRefinement_CISS(PEP pep,PetscInt inner,PetscInt blsize)
{
  ...
  if (inner == PETSC_DEFAULT) {
    ...
    PetscCheckFalse(inner<0,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The refine inner argument must be >= 0");
    ctx->refine_inner = inner;
  }
  if (blsize == PETSC_DEFAULT) {
    ctx->refine_blocksize = 0;
  } else {
    PetscCheckFalse(blsize<0,PetscObjectComm((PetscObject)pep),PETSC_ERR_ARG_OUTOFRANGE,"The refine blocksize argument must be >= 0");
...
static PetscErrorCode PEPCISSGetKSPs_CISS(PEP pep,PetscInt *nsolve,KSP **ksp)
{
  ...
  if (!contour->ksp) {
    ...
    ierr = PEPGetST(pep,&pep->st);CHKERRQ(ierr);
    ierr = STGetSplitPreconditionerInfo(pep->st,&nsplit,NULL);CHKERRQ(ierr);
...
PetscErrorCode PEPView_CISS(PEP pep,PetscViewer viewer)
{
  ...
  if (isascii) {
    ierr = PetscViewerASCIIPrintf(viewer,"  sizes { integration points: %" PetscInt_FMT ", block size: %" PetscInt_FMT ", moment size: %" PetscInt_FMT ", partitions: %" PetscInt_FMT ", maximum block size: %" PetscInt_FMT " }\n",ctx->N,ctx->L,ctx->M,ctx->npart,ctx->L_max);CHKERRQ(ierr);
    if (ctx->isreal) {
    ...
    ierr = PetscViewerASCIIPrintf(viewer,"  iterative refinement  { inner: %" PetscInt_FMT ", blocksize: %" PetscInt_FMT " }\n",ctx->refine_inner, ctx->refine_blocksize);CHKERRQ(ierr);
