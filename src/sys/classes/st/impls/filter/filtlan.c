...
static PetscErrorCode FILTLAN_HermiteBaseFilterInChebyshevBasis(PetscReal *baseFilter,PetscReal *intv,PetscInt npoints,const PetscInt *HighLowFlags,PetscInt baseDeg)
{
  ...
  CHKERRQ(PetscMalloc5(m,&px,m,&py,m,&pp,m,&qq,m,&work));
  sbf = baseFilter;
  ...
  while (jj--) {  /* the main loop to compute the Chebyshev coefficients */
    ...
    if (flag == -1.0) {  /* flag == -1 means that the current interval is a transition polynomial */
      ...
      CHKERRQ(FILTLAN_NewtonPolynomial(m,px,py,pp,work));
      ...
      CHKERRQ(FILTLAN_ExpandNewtonPolynomialInChebyshevBasis(m,aa,bb,pp,px,qq,work));
  ...
  CHKERRQ(PetscFree5(px,py,pp,qq,work));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode FILTLAN_GetIntervals(PetscReal *intervals,PetscReal *frame,PetscInt polyDeg,PetscInt baseDeg,FILTLAN_IOP opts,FILTLAN_PFI filterInfo)
{
  ...
  /* allocate matrices */
  CHKERRQ(PetscMalloc2((polyDeg+2)*(npoints-1),&polyFilter,(2*baseDeg+2)*(npoints-1),&baseFilter));
  ...
  for (numIter=1;numIter<=opts->maxOuterIter;numIter++) {
    ...
    if (filterInfo->filterType == 2) {  /* a <= z1 < (a1) */
      ...
      CHKERRQ(FILTLAN_HermiteBaseFilterInChebyshevBasis(baseFilter,intv,6,HighLowFlags,baseDeg));
      CHKERRQ(FILTLAN_FilteredConjugateResidualPolynomial(polyFilter,baseFilter,2*baseDeg+2,intv,6,opts->intervalWeights,polyDeg));
      ...
      CHKERRQ(FILTLAN_HermiteBaseFilterInChebyshevBasis(baseFilter,intv,4,HighLowFlags,baseDeg));
      CHKERRQ(FILTLAN_FilteredConjugateResidualPolynomial(polyFilter,baseFilter,2*baseDeg+2,intv,4,opts->intervalWeights,polyDeg));
    }
  ...
  CHKERRQ(PetscFree2(polyFilter,baseFilter));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode FILTLAN_FilteredConjugateResidualPolynomial(PetscReal *cpol,PetscReal *baseFilter,PetscInt nbase,PetscReal *intv,PetscInt m,PetscReal *intervalWeights,PetscInt niter)
{
  ...
  CHKERRQ(PetscCalloc4(ld*nintv,&ppol,ld*nintv,&rpol,ld*nintv,&appol,ld*nintv,&arpol));
  CHKERRQ(PetscArrayzero(cpol,ld*nintv));
  ...
  sappol = 3;
  sarpol = 3;
  CHKERRQ(FILTLAN_PiecewisePolynomialInChebyshevBasisMultiplyX(ppol,sppol,ld,intv,nintv,appol,ld));
  ...
  for (i=0;i<niter;i++) {
    ...
    srpol++;
    scpol++;
    CHKERRQ(Mat_AXPY_BLAS(srpol,nintv,-alp0,appol,ld,1.0,rpol,ld));
    CHKERRQ(Mat_AXPY_BLAS(scpol,nintv,-alp,appol,ld,1.0,cpol,ld));
    if (i+1 == niter) break;
    sarpol++;
    CHKERRQ(FILTLAN_PiecewisePolynomialInChebyshevBasisMultiplyX(rpol,srpol,ld,intv,nintv,arpol,ld));
    rho0 = rho;
    rho = FILTLAN_PiecewisePolynomialInnerProductInChebyshevBasis(rpol,srpol,nintv,ld,arpol,sarpol,nintv,ld,intervalWeights);
    bet  = rho / rho0;
    sppol++;
    sappol++;
    CHKERRQ(Mat_AXPY_BLAS(sppol,nintv,1.0,rpol,ld,bet,ppol,ld));
    CHKERRQ(Mat_AXPY_BLAS(sappol,nintv,1.0,arpol,ld,bet,appol,ld));
  }
  CHKERRQ(PetscFree4(ppol,rpol,appol,arpol));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode FILTLAN_FilteredConjugateResidualMatrixPolynomialVectorProduct(Mat A,Vec b,Vec x,PetscReal *baseFilter,PetscInt nbase,PetscReal *intv,PetscInt nintv,PetscReal *intervalWeights,PetscInt niter,Vec *work)
{
  ...
  CHKERRQ(PetscCalloc5(ld*nintv,&ppol,ld*nintv,&rpol,ld*nintv,&cpol,ld*nintv,&appol,ld*nintv,&arpol));
  /* initialize polynomial ppol to be 1 (i.e. multiplicative identity) in all intervals */
  sppol = 2;
  srpol = 2;
  scpol = 2;
  ...
  CHKERRQ(VecCopy(b,r));     /* initial residual r = b-A*x0 */
  CHKERRQ(VecCopy(r,p));     /* p = r */
  CHKERRQ(MatMult(A,p,ap));  /* ap = A*p */
  ...
  for (i=0;i<niter;i++) {
    ...
    alpha = alp;
    CHKERRQ(VecAXPY(x,alpha,p));   /* x += alp*p */
    ...
    alpha = -alp0;
    CHKERRQ(VecAXPY(r,alpha,ap));    /* r -= alp0*ap */
    alpha = bet;
    CHKERRQ(VecAYPX(p,alpha,r));     /* p = r + bet*p */
    CHKERRQ(MatMult(A,r,w));         /* ap = A*r + bet*ap */
    CHKERRQ(VecAYPX(ap,alpha,w));
  }
  CHKERRQ(PetscFree5(ppol,rpol,cpol,appol,arpol));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatMult_FILTLAN(Mat A,Vec x,Vec y)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(MatShellGetContext(A,&st));
  ctx = (ST_FILTER*)st->data;
  npoints = (ctx->filterInfo->filterType == 2)? 6: 4;
  CHKERRQ(FILTLAN_FilteredConjugateResidualMatrixPolynomialVectorProduct(ctx->T,x,y,ctx->baseFilter,2*ctx->baseDegree+2,ctx->intervals,npoints-1,ctx->opts->intervalWeights,ctx->polyDegree,st->work));
  CHKERRQ(VecCopy(y,st->work[0]));
  CHKERRQ(MatMult(ctx->T,st->work[0],y));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode FILTLAN_FilteredConjugateResidualMatrixPolynomialVectorProductBlock(Mat A,Mat B,Mat C,PetscReal *baseFilter,PetscInt nbase,PetscReal *intv,PetscInt nintv,PetscReal *intervalWeights,PetscInt niter,Vec *work,Mat R,Mat P,Mat AP,Mat W)
{
  ...
  CHKERRQ(MatCopy(B,R,SAME_NONZERO_PATTERN));     /* initial residual r = b-A*x0 */
  CHKERRQ(MatCopy(R,P,SAME_NONZERO_PATTERN));     /* p = r */
  CHKERRQ(MatMatMult(A,P,MAT_REUSE_MATRIX,PETSC_DEFAULT,&AP));  /* ap = A*p */
  ...
  for (i=0;i<niter;i++) {
    ...
    /* update x in the vector space */
    alpha = alp;
    CHKERRQ(MatAXPY(C,alpha,P,SAME_NONZERO_PATTERN));   /* x += alp*p */
    if (rho < tol*rho00) break;
    ...
    /* finish the iteration in the vector space */
    alpha = -alp0;
    CHKERRQ(MatAXPY(R,alpha,AP,SAME_NONZERO_PATTERN));    /* r -= alp0*ap */
    alpha = bet;
    CHKERRQ(MatAYPX(P,alpha,R,SAME_NONZERO_PATTERN));     /* p = r + bet*p */
    CHKERRQ(MatMatMult(A,R,MAT_REUSE_MATRIX,PETSC_DEFAULT,&W));         /* ap = A*r + bet*ap */
    CHKERRQ(MatAYPX(AP,alpha,W,SAME_NONZERO_PATTERN));
  }
  CHKERRQ(PetscFree5(ppol,rpol,cpol,appol,arpol));
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatMatMult_FILTLAN(Mat A,Mat B,Mat C,void *pctx)
{
  ...
  if (ctx->nW) {  /* check if work matrices must be resized */
    CHKERRQ(MatGetSize(B,NULL,&m1));
    CHKERRQ(MatGetSize(ctx->W[0],NULL,&m2));
    if (m1!=m2) {
  ...
  if (!ctx->nW) {  /* allocate work matrices */
    ctx->nW = 4;
    CHKERRQ(PetscMalloc1(ctx->nW,&ctx->W));
    CHKERRQ(PetscLogObjectMemory((PetscObject)st,ctx->nW*sizeof(Mat)));
    for (i=0;i<ctx->nW;i++) {
      CHKERRQ(MatDuplicate(B,MAT_DO_NOT_COPY_VALUES,&ctx->W[i]));
    }
  }
  CHKERRQ(FILTLAN_FilteredConjugateResidualMatrixPolynomialVectorProductBlock(ctx->T,B,ctx->W[0],ctx->baseFilter,2*ctx->baseDegree+2,ctx->intervals,npoints-1,ctx->opts->intervalWeights,ctx->polyDegree,st->work,C,ctx->W[1],ctx->W[2],ctx->W[3]));
  CHKERRQ(MatMatMult(ctx->T,ctx->W[0],MAT_REUSE_MATRIX,PETSC_DEFAULT,&C));
  PetscFunctionReturn(0);
}
...
PetscErrorCode STFilter_FILTLAN_setFilter(ST st,Mat *G)
{
  ...
  if (ctx->frame[0] == ctx->frame[1]) {  /* low pass filter, convert it to high pass filter */
    ...
    CHKERRQ(MatDuplicate(st->A[0],MAT_COPY_VALUES,&ctx->T));
    CHKERRQ(MatScale(ctx->T,-1.0));
    alpha = ctx->frame[3];
    CHKERRQ(MatShift(ctx->T,alpha));
    for (i=0;i<4;i++) frame2[i] = ctx->frame[3] - ctx->frame[3-i];
  } else {  /* it can be a mid-pass filter or a high-pass filter */
    if (ctx->frame[0] == 0.0) {
      CHKERRQ(PetscObjectReference((PetscObject)st->A[0]));
      CHKERRQ(MatDestroy(&ctx->T));
      ctx->T = st->A[0];
  ...
  if (st->state==ST_STATE_INITIAL || ctx->filtch) {
    CHKERRQ(FILTLAN_GetIntervals(ctx->intervals,frame2,ctx->polyDegree,ctx->baseDegree,ctx->opts,ctx->filterInfo));
    ...
    CHKERRQ(PetscMalloc1((2*ctx->baseDegree+2)*(npoints-1),&ctx->baseFilter));
    CHKERRQ(FILTLAN_HermiteBaseFilterInChebyshevBasis(ctx->baseFilter,ctx->intervals,npoints,HighLowFlags,ctx->baseDegree));
    CHKERRQ(PetscInfo(st,"Computed value of yLimit = %g\n",(double)ctx->filterInfo->yLimit));
  }
  ctx->filtch = PETSC_FALSE;

  /* create shell matrix*/
  if (!*G) {
    CHKERRQ(MatGetSize(ctx->T,&N,&M));
    CHKERRQ(MatGetLocalSize(ctx->T,&n,&m));
    CHKERRQ(MatCreateShell(PetscObjectComm((PetscObject)st),n,m,N,M,st,G));
    CHKERRQ(MatShellSetOperation(*G,MATOP_MULT,(void(*)(void))MatMult_FILTLAN));
    CHKERRQ(MatShellSetMatProductOperation(*G,MATPRODUCT_AB,NULL,MatMatMult_FILTLAN,NULL,MATDENSE,MATDENSE));
    CHKERRQ(MatShellSetMatProductOperation(*G,MATPRODUCT_AB,NULL,MatMatMult_FILTLAN,NULL,MATDENSECUDA,MATDENSECUDA));
    CHKERRQ(PetscLogObjectParent((PetscObject)st,(PetscObject)*G));
  }
  PetscFunctionReturn(0);
}
