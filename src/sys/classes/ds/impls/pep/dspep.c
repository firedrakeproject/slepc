...
static PetscErrorCode DSPEPSetDegree_PEP(DS ds,PetscInt d)
{
  ...
  if (d>=DS_NUM_EXTRA) SETERRQ1(PetscObjectComm((PetscObject)ds),PETSC_ERR_ARG_OUTOFRANGE,"Only implemented for polynomials of degree at most %" PetscInt_FMT,DS_NUM_EXTRA-1);
  ctx->d = d;
  PetscFunctionReturn(0);
}
