...
/* **************************************************************************** */
  ...
  *(unsigned char *)defl = '0';
  ...
  ierr = BDC_dsrtdf_(&k, n, n1, ev, q, ldq, indxq, rho, &work[iz],
          ...
          CHKERRQ(ierr);
  if (*info) SETERRQ1(PETSC_COMM_SELF,1,"dmerg2: error in dsrtdf, info = %" PetscBLASInt_FMT,*info);
...
/* **************************************************************************** */
  ...
  if (k != 0) {
    ...
    if (spneed > lwork) SETERRQ1(PETSC_COMM_SELF,1,"dmerg2: Workspace needed exceeds the workspace provided by %" PetscBLASInt_FMT " numbers",spneed-lwork);
    ...
    if (*info) SETERRQ1(PETSC_COMM_SELF,1,"dmerg2: error in dlaed3m, info = %" PetscBLASInt_FMT,*info);
