...
PetscErrorCode VecDuplicateVecs_Comp(Vec w,PetscInt m,Vec *V[])
{
  ...
  if (m<=0) SETERRQ(PetscObjectComm((PetscObject)w),PETSC_ERR_ARG_OUTOFRANGE,"m must be > 0: m = %" PetscInt_FMT,m);
...
static PetscErrorCode VecCompSetSubVecs_Comp(Vec win,PetscInt n,Vec *x)
{
  ...
  if (!s->nx) {
    ...
    if (N%n) SETERRQ(PetscObjectComm((PetscObject)win),PETSC_ERR_SUP,"Global dimension %" PetscInt_FMT " is not divisible by %" PetscInt_FMT,N,n);
    ierr = VecGetLocalSize(win,&nlocal);CHKERRQ(ierr);
    if (nlocal%n) SETERRQ(PetscObjectComm((PetscObject)win),PETSC_ERR_SUP,"Local dimension %" PetscInt_FMT " is not divisible by %" PetscInt_FMT,nlocal,n);
    s->nx = n;
    for (i=0;i<n;i++) {
  ...
  } else if (n > s->nx) SETERRQ(PetscObjectComm((PetscObject)win),PETSC_ERR_SUP,"Number of child vectors cannot be larger than %" PetscInt_FMT,s->nx);
  if (x) {
