/*
   ...
   Algorithm:
       ...
       NLEIGS for nonlinear eigenproblems.

   References:

       [1] S. Güttel, "NLEIGS:A class of robust fully rational Krylov method for nonlinear eigenvalue problems",
           BIT .
...
PetscErrorCode NEPSetUp_NLEIGS(NEP nep)
{
  ...
  if (!nep->split) SETERRQ(PetscObjectComm((PetscObject)nep),PETSC_ERR_SUP,"NLEIGS only available for split operator");
