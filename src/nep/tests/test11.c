...
int main(int argc,char **argv)
{
  ...
  CHKERRQ(NEPSetType(nep,NEPCISS));
  CHKERRQ(NEPSetDimensions(nep,1,24,PETSC_DEFAULT));
  CHKERRQ(NEPSetTolerances(nep,1e-9,PETSC_DEFAULT));
  CHKERRQ(NEPGetRG(nep,&rg));
  CHKERRQ(RGSetType(rg,RGELLIPSE));
  CHKERRQ(RGEllipseSetParameters(rg,10.0,9.5,0.1));
  CHKERRQ(NEPCISSSetSizes(nep,PETSC_DEFAULT,PETSC_DEFAULT,PETSC_DEFAULT,1,PETSC_DEFAULT,PETSC_TRUE));
  CHKERRQ(NEPCISSGetKSPs(nep,&nsolve,&ksp));
  for (i=0;i<nsolve;i++) {
    CHKERRQ(KSPSetTolerances(ksp[i],1e-12,PETSC_DEFAULT,PETSC_DEFAULT,PETSC_DEFAULT));
    CHKERRQ(KSPSetType(ksp[i],KSPBCGS));
  ...
  CHKERRQ(PetscObjectTypeCompare((PetscObject)nep,NEPCISS,&flg));
  if (flg) {
    CHKERRQ(NEPCISSGetExtraction(nep,&ext));
    CHKERRQ(PetscPrintf(PETSC_COMM_WORLD," Running CISS with %" PetscInt_FMT " KSP solvers (%s extraction)\n",nsolve,NEPCISSExtractions[ext]));
  }
