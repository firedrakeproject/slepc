...
#undef __FUNCT__
#define __FUNCT__ "QEPSTOARNorm_private"
/*
  Compute B-norm of v=[v1;v2] whith  B=diag(-qep->T[0],qep->T[2]) 
*/
PetscErrorCode QEPSTOARNorm_private(QEP qep,Vec v1,Vec v2,PetscReal *norm,Vec vw)
{
  PetscErrorCode ierr;
  PetscScalar    p1,p2;
...
#undef __FUNCT__
#define __FUNCT__ "QEPSTOARCGS"
/*
  Compute a step of Classical Gram-Schmidt orthogonalization
*/
static PetscErrorCode QEPSTOARCGS(QEP qep,PetscScalar *H,PetscBLASInt ldh,PetscReal *omega,PetscScalar *h,PetscBLASInt j,Vec *V,Vec t,Vec v,Vec w,PetscReal *onorm,PetscReal *norm,PetscScalar *work,Vec vw)
{
  PetscErrorCode ierr;
  ...
  /* compute norm of [v;w] */
  if (onorm) {
    ierr = QEPSTOARNorm_private(qep,v,w,onorm,vw);CHKERRQ(ierr);
  }
  ...
  if (norm) {
    ierr = QEPSTOARNorm_private(qep,v,w,norm,vw);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
...
*/
static PetscErrorCode QEPSTOARrun(QEP qep,PetscScalar *H,PetscInt ldh,PetscReal *omega,Vec *V,PetscInt k,PetscInt *M,Vec v,Vec w,PetscBool *breakdown,PetscScalar *work,Vec *t_)
{
  PetscErrorCode     ierr;
  ...
  for (j=k;j<m;j++) {
    ...
    switch (refinement) {
      case IP_ORTHOG_REFINE_NEVER:
        ierr = QEPSTOARCGS(qep,H,ldh,omega,H+ldh*j,j,V,t,v,w,NULL,&norm,work,u);CHKERRQ(ierr);
        break;
      case IP_ORTHOG_REFINE_ALWAYS:
        ierr = QEPSTOARCGS(qep,H,ldh,omega,H+ldh*j,j,V,t,v,w,NULL,NULL,work,u);CHKERRQ(ierr);
        ierr = QEPSTOARCGS(qep,H,ldh,omega,c,j,V,t,v,w,&onorm,&norm,work,u);CHKERRQ(ierr);
        for (i=0;i<=j;i++) H[ldh*j+i] += c[i];
        break;
      case IP_ORTHOG_REFINE_IFNEEDED: /* -- */
        ierr = QEPSTOARCGS(qep,H,ldh,omega,H+ldh*j,j,V,t,v,w,&onorm,&norm,work,u);CHKERRQ(ierr);
        /* ||q|| < eta ||h|| */
        l = 1;
        while (l<3 && PetscAbsReal(norm) < eta * PetscAbsReal(onorm)) {
          l++;
          onorm = norm;
          ierr = QEPSTOARCGS(qep,H,ldh,omega,c,j,V,t,v,w,NULL,&norm,work,u);CHKERRQ(ierr);
          for (i=0;i<=j;i++) H[ldh*j+i] += c[i];
        }
        break;
...
PetscErrorCode QEPSolve_STOAR(QEP qep)
{
  ...
  ierr = QEPSTOARNorm_private(qep,qep->V[0],w,&norm,w_);CHKERRQ(ierr);
  ...
  while (qep->reason == QEP_CONVERGED_ITERATING) {
    ...
    ierr = QEPSTOARrun(qep,S,ld,omega,qep->V,qep->nconv+l,&nv,v,w,&breakdown,work,&qep->work[2]);CHKERRQ(ierr);
    for (j=ti;j<nv-1;j++) {
