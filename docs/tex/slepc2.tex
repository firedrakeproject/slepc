...
\end{figure}
	...
	\end{Verbatim}
	The subroutine which is actually invoked depends on which solver has been selected by the user. On return, the parameter \texttt{its} contains either the iteration number at which convergence was successfully reached, or the \emph{negative} of the iteration at which a problem was detected. 
	...
	\end{Verbatim}
	queries how many eigenpairs have converged to working precision. The solution of the eigenproblem is retrieved in line 13 with a call to the following function
	\findex{EPSGetSolution}
	\begin{Verbatim}[fontsize=\small]
	EPSGetSolution(EPS eps,PetscScalar **kr,PetscScalar **ki,Vec **x);
	\end{Verbatim}
	\label{GetSolution}
	This function returns pointers to the solution of the problem, which is stored in data structures allocated internally by the \ident{EPS} code. \texttt{kr} and \texttt{ki} point to arrays containing the real and imaginary parts of the eigenvalues, while \texttt{x} points to an array of \texttt{Vec} objects which contain the associated eigenvectors. Therefore, the $j$-th eigenvalue is \texttt{kr[j]}$+\,i\cdot$\texttt{ki[j]} and the $j$-th eigenvector is stored in the \texttt{Vec} objects \texttt{x[j]} and \texttt{x[j+1]}. 
	[Note: see section \ref{sec:complex} for a detailed discussion of this issue.] 
	
	The reason why the user does not need to allocate memory for the solution of the eigenproblem is that the amount of required storage usually depends on the number of basis vectors and the number of requested eigenvalues, which are parameters typically specified at run time. The implemented solution automatically allocates the necessary memory space and returns a pointer to this space. The user is warned not to use these objects after the \ident{EPS} object has been destroyed (see below).
	
	In line 15 of the example the relative residual error $\|Ax_j\!-\!\lambda_jBx_j\|/|\lambda_j|$ associated to each eigenpair is computed with a call to
	\findex{EPSComputeError}
	\begin{Verbatim}[fontsize=\small]
	EPSComputeError(EPS eps,PetscReal *error);
	\end{Verbatim}
	In this case, the user is responsible for allocating an array to store the computed errors.
	...
	The above procedure is sufficient for general use of the \ident{EPS} package. As in the case of the \ident{SLES} solver, the user can optionally explicitly call 
	\findex{EPSSetUp}
	\begin{Verbatim}[fontsize=\small]
	EPSSetUp(EPS eps);
	\end{Verbatim}
...
\texttt{EPSWhich}              & Command line key\\\hline
\texttt{EPS\_LARGEST\_MAGNITUDE}   & \texttt{-eps\_largest\_magnitude}\\
\texttt{EPS\_SMALLEST\_MAGNITUDE}  & \texttt{-eps\_smallest\_magnitude}\\
\texttt{EPS\_LARGEST\_ALGEBRAIC}   & \texttt{-eps\_largest\_algebraic}\\
\texttt{EPS\_SMALLEST\_ALGEBRAIC}  & \texttt{-eps\_smallest\_algebraic}\\
\texttt{EPS\_LARGEST\_REAL}        & \texttt{-eps\_largest\_real}\\
\texttt{EPS\_SMALLEST\_REAL}       & \texttt{-eps\_smallest\_real}\\
\texttt{EPS\_LARGEST\_IMAGINARY}   & \texttt{-eps\_largest\_imaginary}\\
\texttt{EPS\_SMALLEST\_IMAGINARY}  & \texttt{-eps\_smallest\_imaginary}\\
\texttt{EPS\_BOTH\_ENDS}           & \texttt{-eps\_both\_ends}\\\hline
\end{tabular} }
...
The \ident{EPS} method can also be set with the options database command \Verb!-eps_type! followed by the name of the method (see table \ref{tab:solvers}).
	...
	It is possible to choose among several variants of the algorithms. For instance, in many methods one can select the orthogonalization technique to be used. Again, this can be done via the following command line or procedurally. Available choices are listed in table \ref{tab:orth}.
