...
\paragraph{Complex \slepc.} In this case, all \texttt{Mat} and \texttt{Vec} objects are complex. The computed approximate solution returned by the function \ident{EPSGetSolution} is the following: \texttt{kr} contains the (complex) eigenvalues and \texttt{x} contains the corresponding (complex) eigenvectors. In this case, \texttt{ki} is not used.
