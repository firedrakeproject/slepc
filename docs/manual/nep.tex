...
Method             & \ident{NEPType}      & {\footnotesize Database} & {\footnotesize $T'(\cdot)$} & {\footnotesize Eigenvalues} \\\hline
Residual inverse iteration & \texttt{NEPRII}      & \texttt{rii}      & no  & only one\\
Successive linear problems & \texttt{NEPSLP}      & \texttt{slp}      & yes & only one\\
Nonlinear Arnoldi          & \texttt{NEPNARNOLDI} & \texttt{narnoldi} & no  & only one\\
Rational Krylov (NLEIGS)   & \texttt{NEPNLEIGS}   & \texttt{nleigs}   & no  & many\\
