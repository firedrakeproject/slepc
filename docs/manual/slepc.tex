...
\item Added chapter \ref{cap:nep} for nonlinear eigenvalue problems.
\item Added chapter \ref{cap:mfn} for matrix functions.
\item Added section \ref{sec:qst} on spectral transformation for quadratic eigenproblems.
\end{itemize}
...
\subsubsection*{Acknowledgments}
	...
	The current version contains code contributed by: Tetsuya Sakurai.
...
\begin{quote}
\begin{sffamily}
Copyright 2002--2013 Universidad Polit\'ecnica de Valencia, Spain
\end{sffamily}
\end{quote}
...
Quadratic eigenvalue problem  & $(\lambda^2M+\lambda C+K)x=0$ & \texttt{QEP} & \ref{cap:qep} \\
Polynomial eigenvalue problem & $(\lambda^dA_d+\cdots+\lambda A_1+A_0)x=0$ & -- & -- \\
...
\item There are no polynomial eigensolvers available yet. For future releases, we foresee having a \texttt{PEP} module replacing \texttt{QEP}.
