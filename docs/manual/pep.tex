...
The problem type can be specified at run time with the corresponding command line key or, more usually, within the program with the function
	...
	\end{Verbatim}
Currently, problem types are considered in some solvers for the case of quadratic problems only.
...
The solution method can be specified procedurally with
	...
	\end{Verbatim}
or via the options database command \Verb!-pep_type! followed by the name of the method. The methods currently available in \ident{PEP} are listed in Table~\ref{tab:solversq}. The first one is based on linearization, while the rest either work implicitly on a linearization, or operate directly on the quadratic problem without linearizing.

\begin{table}
\centering
{\small \begin{tabular}{lll}
                   &                      & {\footnotesize Options} \\
Method             & \ident{PEPType}      & {\footnotesize Database Name}\\\hline
Linearization      & \texttt{PEPLINEAR}   & \texttt{linear} \\
Quadratic Arnoldi  & \texttt{PEPQARNOLDI} & \texttt{qarnoldi} \\\hline
\end{tabular} }
\caption{\label{tab:solversq}Quadratic eigenvalue solvers available in the \ident{PEP} module.}
\end{table}

The \texttt{PEPLINEAR} method carries out a linearization of the quadratic eigenproblem, as described in \S\ref{sec:qep}, resulting in a generalized eigenvalue problem that is handled by an \ident{EPS} object created internally. If required, this \ident{EPS} object can be extracted with the operation
...
\begin{Verbatim}[fontsize=\small]
	$ ./program -pep_type linear -pep_hermitian -pep_linear_cform 2
                    -pep_linear_explicitmatrix -pep_eps_type krylovschur
                    -pep_st_ksp_type gmres -pep_st_pc_type bjacobi
\end{Verbatim}
...
For computing eigenvalues in the interior of the spectrum (closest to a target $\tau$), it is necessary to use a spectral transformation. Currently, only shift-and-invert is supported in \ident{PEP} solvers, and the way to handle it is via an \ident{ST} object as in the case of linear eigensolvers. Every \ident{PEP} object has an \ident{ST} object internally. Note that it would also be possible to select a spectral transformation in the \ident{ST} contained in the \ident{EPS} solver for the \texttt{PEPLINEAR} case. However, we do not recommend this approach.
...
with $\mu=\lambda/\alpha$, $M_\alpha=\alpha^2M$ and $C_\alpha=\alpha C$, where $\alpha$ is a scaling factor. Ideally, $\alpha$ should be chosen in such a way that the norms of $M_\alpha$, $C_\alpha$ and $K$ have similar magnitude. In \slepc, this is done automatically and the user can call
	\findex{PEPSetScaleFactor}
	\begin{Verbatim}[fontsize=\small]
	PEPSetScaleFactor(PEP pep,PetscReal alpha);
	\end{Verbatim}
