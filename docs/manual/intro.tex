...
\section{SLEPc and PETSc}
	...
	\slepc can be considered an extension of \petsc providing all the functionality necessary for the solution of eigenvalue problems. Figure \ref{fig:slepc} shows a diagram of all the different objects included in \petsc (on the left) and those added by \slepc (on the right). \petsc is a prerequisite for \slepc and users should be familiar with basic concepts such as vectors and matrices in order to use \slepc. Therefore, together with this manual we recommend to use the \petsc Users Manual \citep{Balay:2014:PUM}.
...
\begin{tikzpicture}[xscale=0.7,yscale=0.8] 
  ...
  \draw[implem] (11,2*\levsep) rectangle node {Krylov-Schur} ++(2.5,1) ++(0,-1) 
                rectangle node {GD} ++(1.0,1) ++(0,-1)
                rectangle node {JD} ++(1.0,1) ++(0,-1)
                rectangle node {RQCG} ++(1.4,1) ++(0,-1)
...
\begin{enumerate}
	...
	\begin{Verbatim}[fontsize=\small]
	$ export SLEPC_DIR=/home/username/slepc-3.5.0
	\end{Verbatim}
	...
	\begin{Verbatim}[fontsize=\small]
	$ export PETSC_DIR=/home/username/petsc-3.5.0
...
To be more precise, suppose that the configuration was done with \texttt{-{}-prefix=/opt/petsc-3.5.0-linux-gnu-c-debug}. Then, \texttt{make install} will create directory \texttt{/opt/petsc-3.5.0-linux-gnu-c-debug} if it does not exist, and create subdirectories \texttt{lib}, \texttt{conf}, and \texttt{include}, that will store the libraries, the configuration files, and the header files, respectively. Note that the source code files are not copied, nor the documentation, so the size of the installed directory will be much smaller than the original one. For that reason, it is no longer necessary to allow for several configurations to share a directory tree. In other words, in a prefix-based installation, variable \ident{PETSC\_ARCH} loses significance and must be unset. To maintain several configurations, one should specify different prefix directories, typically with a name that informs about the configuration options used.
...
In order to prepare a prefix-based installation of \slepc that uses a prefix-based installation of \petsc, start by setting the appropriate value of \ident{PETSC\_DIR}. Then, run \slepc's configure with a prefix directory.
	...
	$ export PETSC_DIR=/opt/petsc-3.5.0-linux-gnu-c-debug
	$ unset PETSC_ARCH
	$ cd $SLEPC_DIR
	$ ./configure --prefix=/opt/slepc-3.5.0-linux-gnu-c-debug
	$ make
	$ make install
	$ export SLEPC_DIR=/opt/slepc-3.5.0-linux-gnu-c-debug
	\end{Verbatim}
Note that it is important to unset the value of \ident{PETSC\_ARCH} before \slepc's configure. \slepc will use a temporary arch name during the build. Although it is not a common case, it is also possible to configure \slepc without prefix, in which case the \ident{PETSC\_ARCH} variable is again necessary (must be set to the value reported by \slepc's configure).
...
\ident{SLEPC\_DIR}, indicating the full path of the directory containing \slepc. For example, under the \texttt{bash} shell, a command of the form
	\begin{Verbatim}[fontsize=\small]
	$ export SLEPC_DIR=/software/slepc-3.5.0
	\end{Verbatim}
