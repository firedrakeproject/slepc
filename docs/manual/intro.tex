...
In order to prepare a prefix-based installation of \slepc that uses a prefix-based installation of \petsc, start by setting the appropriate value of \ident{PETSC\_DIR}. Then, run \slepc's configure with a prefix directory.
	...
	\end{Verbatim}
Note that it is important to unset the value of \ident{PETSC\_ARCH} before \slepc's configure. The special value \texttt{installed-petsc} of the \ident{PETSC\_ARCH} variable is required only during \slepc's build, and should not be used afterwards.
