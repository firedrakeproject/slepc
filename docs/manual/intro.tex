...
\section{SLEPc and PETSc}
	...
	\slepc is based on \petsc, the Portable, Extensible Toolkit for Scientific Computation \citep{Balay:2002:PUM}, and, therefore, a large percentage of the software complexity is avoided since many \petsc developments are leveraged, including matrix storage formats and linear solvers, to name a few. \slepc focuses on high level features for eigenproblems, structured around a few object types as described below. 
...
\begin{tikzpicture}[xscale=0.7,yscale=0.8] 
  ...
  \draw[interface] (0,0) rectangle node {Vectors} +(2.7,1.6);
  \draw[interface] (3.5,1) rectangle node {Index Sets} +(7,0.6);
  \draw[implem] (3.5,0) rectangle node {Indices} ++(1.5,1) ++(0,-1) 
                rectangle node {Block Indices} ++(2.5,1) ++(0,-1)
                rectangle node {Stride} ++(1.5,1) ++(0,-1)
                rectangle node {Other} ++(1.5,1);
  \draw[interface] (0,2.85) rectangle node {Matrices} +(10.5,0.6);
  \draw[implem] (0,1.85) rectangle node[text width=2.5cm] {Compressed Sparse Row (AIJ)} ++(2.7,1) ++(0,-1) 
                rectangle node[text width=3cm] {Block Compressed Sparse Row (BAIJ)} ++(3,1) ++(0,-1) 
                rectangle node[text width=2.2cm] {Block Diagonal (BDIAG)} ++(2.2,1) ++(0,-1) 
  ...
  \draw[implem] (4.5,7.4) rectangle node {Euler} ++(1.1,1) ++(0,-1) 
                ...
                rectangle node {Other} ++(1.1,1);
  \node[above,text centered,font=\sffamily\bfseries\Large] at (13.85,9.1) {SLEPc};
  \draw[interface] (11,8.4) rectangle node {SVD Solvers} +(5.7,0.6);
  \draw[implem] (11,7.4) rectangle node[text width=1.2cm] {Cross Product} ++(1.3,1) ++(0,-1) 
                ...
                rectangle node {Lanczos} ++(1.5,1) ++(0,-1) 
                rectangle node[text width=1.7cm] {Thick Res. Lanczos} ++(1.7,1);
  \draw[interface] (11,6.55) rectangle node {Eigensolvers} +(5.7,0.6);
  \draw[implem] (11,5.55) rectangle node {Krylov-Schur} ++(2,1) ++(0,-1) 
                rectangle node {Arnoldi} ++(1.3,1) ++(0,-1) 
                rectangle node {Lanczos} ++(1.3,1) ++(0,-1) 
                rectangle node {Other} ++(1.1,1);
  \draw[interface] (11,4.7) rectangle node {Spectral Transform} +(5.7,0.6);
  \draw[implem] (11,3.7) rectangle node {Shift} ++(1,1) ++(0,-1) 
                rectangle node {Shift-and-invert} ++(2.5,1) ++(0,-1) 
                rectangle node {Cayley} ++(1.2,1) ++(0,-1) 
                rectangle node {Fold} ++(1,1);
\end{tikzpicture}
...
\slepc version &     \petsc versions &  Major  & Release date \\ \hline
         ...
         3.0.0 &               3.0.0 & $\star$ & Feb 2009     \\ \hline
\end{tabular}
...
\begin{enumerate}
	...
	\begin{Verbatim}[fontsize=\small]
	export PETSC_DIR=/home/username/petsc-3.0.0
	export PETSC_ARCH=darwin9.6.0-c-debug
	\end{Verbatim}
...
\item Build single precision versions with \Verb!--with-precision=single!. In most applications, this can achieve a significant reduction of memory requirements, and a moderate reduction of computing time. Other precisions are available, such as \texttt{longdouble}.
...
\item If not detected, use \Verb!--with-blas-lapack-lib! to specify the location of \lapack. If \slepc's configure complains about some missing \lapack subrotines, reconfigure \petsc with option \Verb!--download-c-blas-lapack=yes!.
\item Enable external libraries that provide direct linear solvers or preconditioners, such as MUMPS, hypre, or SuperLU. For example, \Verb!--download-mumps=ifneeded!. These are especially relevant for \slepc in the case that a spectral transformation is used, see chapter \ref{cap:st}.
...
\item Build shared libraries, \Verb!--with-shared=1!, for smaller executables that are linked at run time.
...
In a standard installation, the different configurations are identified by a unique name that is assigned to the environment variable \ident{PETSC\_ARCH}. Let us illustrate how to set up \petsc with two configurations. First, set a value of \ident{PETSC\_ARCH} and proceed with the installation of the first one:
	\begin{Verbatim}[fontsize=\small]
	cd $PETSC_DIR
	export PETSC_ARCH=linux-gnu-c-debug-real
...
Note that if \ident{PETSC\_ARCH} is not given a value, \petsc suggests one for us. After this, a subdirectory named \texttt{\$PETSC\_ARCH} is created within \texttt{\$PETSC\_DIR}, that stores all information associated to that configuration, including the built libraries, configuration files, automatically generated source files, and log files. For the second configuration, proceed similarly:
	\begin{Verbatim}[fontsize=\small]
	cd $PETSC_DIR
	export PETSC_ARCH=linux-gnu-c-debug-complex
...
To be more precise, suppose that the configuration was done with \texttt{-{}-prefix=/opt/petsc-3.0.0-linux-gnu-c-debug}. Then, \texttt{make install} will create directory \texttt{/opt/petsc-3.0.0-linux-gnu-c-debug} if it does not exist, and create subdirectories \texttt{lib}, \texttt{conf}, and \texttt{include}, that will store the libraries, the configuration files, and the header files, respectively. Note that the source code files are not copied, nor the documentation, so the size of the installed directory will be much smaller than the original one. For that reason, it is no longer necessary to allow for several configurations to share a directory tree. In other words, in a prefix-based installation, variable \ident{PETSC\_ARCH} loses significance and must be unset. To maintain several configurations, one should specify different prefix directories, typically with a name that informs about the configuration options used.

In order to prepare a prefix-based installation of \slepc that uses a prefix-based installation of \petsc, start by setting the appropriate value of \ident{PETSC\_DIR}, making sure that \ident{PETSC\_ARCH} is not set. Then, run \slepc's configure with a prefix directory.
	\begin{Verbatim}[fontsize=\small]
	export PETSC_DIR=/opt/petsc-3.0.0-linux-gnu-c-debug
	unset PETSC_ARCH
	cd $SLEPC_DIR
	./configure --prefix=/opt/slepc-3.0.0-linux-gnu-c-debug
	export PETSC_ARCH=linux-gnu-c-debug
	make 
	make install
	unset PETSC_ARCH
	export SLEPC_DIR=/opt/slepc-3.0.0-linux-gnu-c-debug
	\end{Verbatim}
Note that \slepc's configure will display the value of \ident{PETSC\_ARCH} that has to be used (it is the same name that was used during \petsc's configuration, before install). In this case, the \ident{PETSC\_ARCH} variable is required only during \slepc's build, but should be unset afterwards.
...
\ident{SLEPC\_DIR}, indicating the full path of the directory containing \slepc. For example, under the \texttt{bash} shell, a command of the form
	\begin{Verbatim}[fontsize=\small]
	export SLEPC_DIR=/software/slepc-3.0.0
	\end{Verbatim}
...
The examples provided in the \Verb!src/examples! directory demonstrate the software usage and can serve as templates for developing custom applications. To write a new application program using \slepc, we suggest the following procedure:
\begin{enumerate}
