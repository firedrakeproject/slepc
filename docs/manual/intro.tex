...
\section{SLEPc and PETSc}
	...
	\slepc is based on \petsc, the Portable, Extensible Toolkit for Scientific Computation \citep{Balay:2015:PUM}, and, therefore, a large percentage of the software complexity is avoided since many \petsc developments are leveraged, including matrix storage formats and linear solvers, to name a few. \slepc focuses on high level features for eigenproblems, structured around a few object classes as described below. 
	...
	\slepc can be considered an extension of \petsc providing all the functionality necessary for the solution of eigenvalue problems. Figure \ref{fig:slepc} shows a diagram of all the different objects included in \petsc (on the left) and those added by \slepc (on the right). \petsc is a prerequisite for \slepc and users should be familiar with basic concepts such as vectors and matrices in order to use \slepc. Therefore, together with this manual we recommend to use the \petsc Users Manual \citep{Balay:2015:PUM}.
...
\begin{tikzpicture}[xscale=0.7,yscale=0.8] 
  ...
  \draw[interface] (0,\himp) rectangle node {Vectors} +(3.7,\hint);
  \draw[implem] (0,0) rectangle node {Standard} ++(2.2,1) ++(0,-1)
                      rectangle node {CUSP} ++(1.5,1) ++(0,-1);
  \draw[interface] (4.5,\himp) rectangle node {Index Sets} +(6,\hint);
  ...
  \draw[implem] (0,\levsep) rectangle node[text width=2.5cm] {Compressed Sparse Row} ++(2.6,1) ++(0,-1) 
  ...
  \draw[interface] (11,4*\levsep+\himp) rectangle node {Polynomial Eigensolver} +(3.8,\hint);
  \draw[implem] (11,4*\levsep) rectangle node[text width=1.2cm] {TOAR} ++(1.2,1) ++(0,-1) 
                rectangle node[text width=1.2cm] {Q\-Arnoldi} ++(1.2,1) ++(0,-1)
                rectangle node[text width=1.7cm] {Linear\-ization} ++(1.4,1);

  \draw[interface] (15,4*\levsep+\himp) rectangle node {Nonlinear Eigensolver} +(4.2,\hint);
  \draw[implem] (15,4*\levsep) rectangle node {SLP} ++(1,1) ++(0,-1) 
                rectangle node {RII} ++(.9,1) ++(0,-1)
                rectangle node[text width=1.2cm] {N\-Arnoldi} ++(1.2,1) ++(0,-1)
                rectangle node[text width=1.2cm] {Interp.} ++(1.1,1);
  \draw[interface] (11,3*\levsep+\himp) rectangle node {SVD Solver} +(5.2,\hint);
  \draw[implem] (11,3*\levsep) rectangle node[text width=1.2cm] {Cross Product} ++(1.3,1) ++(0,-1) 
  ...
  \draw[interface] (16.5,3*\levsep+\himp) rectangle node {M.~Function} +(2.7,\hint);
  \draw[implem] (16.5,3*\levsep) rectangle node {Krylov} ++(2.7,1) ++(0,-1);
  \draw[interface] (11,2*\levsep+\himp) rectangle node {Linear Eigensolver} +(8.2,\hint);
  \draw[implem] (11,2*\levsep) rectangle node {Krylov-Schur} ++(2.3,1) ++(0,-1) 
                rectangle node {GD} ++(1.0,1) ++(0,-1)
                rectangle node {JD} ++(1.0,1) ++(0,-1)
                rectangle node {LOBPCG} ++(1.6,1) ++(0,-1)
                rectangle node {CISS} ++(1.2,1) ++(0,-1)
                rectangle node {Other} ++(1.1,1);
  \draw[interface] (11,\levsep+\himp) rectangle node {Spectral Transformation} +(8.2,\hint);
  \draw[implem] (11,\levsep) rectangle node {Shift} ++(1.5,1) ++(0,-1) 
                rectangle node {Shift-and-invert} ++(2.5,1) ++(0,-1) 
                rectangle node {Cayley} ++(1.7,1) ++(0,-1) 
                rectangle node {Preconditioner} ++(2.5,1);
  \draw[interface] (11,\hint) rectangle node {BV} +(1.9,1.0);
  \draw[interface] (13.1,\hint) rectangle node {DS} +(1.9,1.0);
  \draw[interface] (15.2,\hint) rectangle node {RG} +(1.9,1.0);
  \draw[interface] (17.3,\hint) rectangle node {FN} +(1.9,1.0);
\end{tikzpicture}
...
\begin{enumerate}
	...
	\begin{Verbatim}[fontsize=\small]
	$ export SLEPC_DIR=/home/username/slepc-3.6.0
	\end{Verbatim}
	...
	\begin{Verbatim}[fontsize=\small]
	$ export PETSC_DIR=/home/username/petsc-3.6.0
...
In order to prepare a prefix-based installation of \slepc that uses a prefix-based installation of \petsc, start by setting the appropriate value of \ident{PETSC\_DIR}. Then, run \slepc's configure with a prefix directory.
	...
	$ export PETSC_DIR=/opt/petsc-3.6.0-linux-gnu-c-debug
	$ unset PETSC_ARCH
	$ cd $SLEPC_DIR
	$ ./configure --prefix=/opt/slepc-3.6.0-linux-gnu-c-debug
	$ make
	$ make install
	$ export SLEPC_DIR=/opt/slepc-3.6.0-linux-gnu-c-debug
	\end{Verbatim}
...
\ident{SLEPC\_DIR}, indicating the full path of the directory containing \slepc. For example, under the \texttt{bash} shell, a command of the form
	\begin{Verbatim}[fontsize=\small]
	$ export SLEPC_DIR=/software/slepc-3.6.0
	\end{Verbatim}
...
on their selected computer system(s).  Usually, the \texttt{mpiexec} command can be used to initiate a program as in the following example that uses eight processes:
	\begin{Verbatim}[fontsize=\small]
	$ mpiexec -np 8 slepc_program [command-line options]
	\end{Verbatim}
...
All the \petsc functionality related to the options database is available in \slepc. This allows the user to input control data at run time very easily. In this example the command \Verb!PetscOptionsGetInt(NULL,NULL,"-n",&n,NULL);! checks whether the user has provided a command line option to set the value of \Verb!n!, the problem dimension.  If so, the variable \Verb!n! is set accordingly; otherwise, \Verb!n! remains unchanged.
