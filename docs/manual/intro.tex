...
In order to prepare a prefix-based installation of \slepc that uses a prefix-based installation of \petsc, start by setting the appropriate value of \ident{PETSC\_DIR}. Then, run \slepc's configure with a prefix directory.
	...
	\end{Verbatim}
Note that the value of \ident{PETSC\_ARCH} set before \slepc's configure need not be the same as the one used during \petsc's configuration, before install. This value of the \ident{PETSC\_ARCH} variable is required only during \slepc's build, and should be unset afterwards.
