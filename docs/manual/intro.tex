...
\begin{tikzpicture}[xscale=0.7,yscale=0.8] 
  ...
  %\draw[interface] (16.35,\levsep) rectangle node {BV} +(.9,\hint+\himp) ++(1.05,0)
  %                 rectangle node {DS} +(.9,\hint+\himp) ++(1.05,0)
  %                 rectangle node {RG} +(.9,\hint+\himp) ++(1.05,0)
  %                 rectangle node {FN} +(.9,\hint+\himp);

  %\draw[interface] (11,\levsep+\himp) rectangle node {Spectral Transformation} +(9.4,\hint);
  %\draw[implem] (11,\levsep) rectangle node {Shift} ++(1.4,1) ++(0,-1) 
  %              rectangle node {Shift-and-invert} ++(2.5,1) ++(0,-1) 
  %              rectangle node {Cayley} ++(1.6,1) ++(0,-1) 
  %              rectangle node {Preconditioner} ++(2.5,1) ++(0,-1)
  %              rectangle node {Shell} ++(1.4,1);
  %\draw[interface] (11,\hint) rectangle node {BV} +(2.05,1.0);
  %\draw[interface] (13.45,\hint) rectangle node {DS} +(2.05,1.0);
  %\draw[interface] (15.9,\hint) rectangle node {RG} +(2.05,1.0);
  %\draw[interface] (18.35,\hint) rectangle node {FN} +(2.05,1.0);
\end{tikzpicture}
