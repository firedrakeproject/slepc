...
\begin{tikzpicture}[scale=0.5]
  ...
  \node at (6.75,1.75) {=};
  \draw (7.25,1) rectangle +(3,2) (7.25,3) -- +(2,-2);
...
In the case of the GSVD, the function \ident{SVDComputeError} will compute a residual based on the equivalent eigenvalue problem~\eqref{eq:gsvdeig}, that is
\begin{equation}
\|r_\mathrm{GSVD}\|_2=\|A^*A\tilde{x}-\tilde{\sigma}^2B^*B\tilde{x}\|_2,
\end{equation}
where $\tilde{x}$ is the computed right singular vector corresponding to $\tilde{\sigma}$.
