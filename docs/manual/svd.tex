...
Defining the problem consists in specifying the problem matrix, $A$, and the portion of the spectrum to be computed. If the GSVD is to be computed instead of the SVD, then a second matrix must be passed, $B$. In the case of \texttt{SVD}, the number of possibilities will be much more limited than in the case of \texttt{EPS}.
...
\lapack solver             & \texttt{SVDLAPACK}   & \texttt{lapack} \\
